        ...
        <div className="vstack gap-4 overflow-auto hide-scrollbar">
          {loadingFriends ? (
            ...
            friends.map((friend) => (
              <div
                key={friend._id}
                className="p-3 rounded-4 w-100 d-flex align-items-center gap-3 shadow-sm bg-white"
              >
                <img
                ...
                <i
                  className="fa-solid fa-user-minus fs-4 ms-auto"
                  onClick={() => handleRemovingFriend(friend._id)}
                ></i>
              </div>
            ))
          ) : (
            <div className="mt-3 d-flex flex-column justify-content-between align-items-center">
              <h6 className="text-primary text-center">No friends</h6>
            </div>
          )}
        </div>
      </div>

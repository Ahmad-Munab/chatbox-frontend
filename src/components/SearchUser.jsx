...
const SearchUser = () => {
  ...
  return (
    <>
      <i className="fa-regular fa-plus btn plus-icon thin-blue"
          ...
          <div className="modal-content">
            ...
            <div className="modal-body" id="search-model">
              <div className="form-floating">
                ...
                />
                <label htmlFor="username" className="form-label username-input-label">
                  Username
                </label>
                <i className="searchIcon fa-solid fa-magnifying-glass btn" onClick={handleSubmition}/>
              </div>
              {loadingUsers ? (
                ...
                <div className="vstack gap-2 mt-3">
                  {users !== null && users.length > 0 && users
                    .filter((user) =>
                      user.username.toLowerCase().includes(username) || user.handle.toLowerCase().includes(username)
                    )
                    .map((user) => (
                  ...
                  {(users === null || users.filter((user) =>
                    user.username.toLowerCase().includes(username) || user.handle.toLowerCase().includes(username)
                  ).length < 1 )&& `No user named ${username}`}
                </div>
              )}
            </div>
          </div>
